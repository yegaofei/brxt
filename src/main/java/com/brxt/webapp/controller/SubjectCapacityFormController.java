...
public class SubjectCapacityFormController extends BaseFormController {
    ...
    protected void initBinder(final HttpServletRequest request, final ServletRequestDataBinder binder) {
        ...
        SimpleDateFormat checkTimeDateFormat = new SimpleDateFormat(getText("date.format.short", request.getLocale()));
        binder.registerCustomEditor(Date.class, "checkTime", new CustomDateEditor(checkTimeDateFormat, true));
    }

    @RequestMapping(method = RequestMethod.POST)
    public String onSubmit(@ModelAttribute("subjectCapacity") final SubjectCapacity subjectCapacity,
            @ModelAttribute("projectInfoId") final Long projectInfoId, @ModelAttribute("counterpartyId") final Long counterpartyId,
        ...
        if (!StringUtils.isBlank(method)) {
            ...
            switch (method) {
                case "SaveSubjectCapacity" :
                    ...
                    saveMessage(request, getText("subjectCapacity.save.successful", locale));
                    break;
                case "Cancel" :
                    break;
                default :
    ...
    public SubjectCapacity getSubjectCapacity(final HttpServletRequest request) {
        ...
        SubjectCapacity subjectCapacity = null;
        if (!StringUtils.isBlank(subjectCapacityId)) {
            // Edit
            subjectCapacity = subjectCapacityManager.get(Long.valueOf(subjectCapacityId));
        } else {
            // Add
            subjectCapacity = getNewSubjectCapacity();
        }
