...
public class CorpBalanceSheetController extends BaseSheetController {
    ...
    public CorpBalanceSheetModel getCorpBalanceSheetModel(final HttpServletRequest request, final HttpSession session) {
        ...
        if (!StringUtils.isBlank(statementId)) {
            ...
            if (StringUtils.isBlank(reportTime)) {
                ...
                endBalSheet = financeSheetManager.findCorporateBalanceSheet(projectInfo, counterparty, getYear(csm.getReportTime()),
                        getMonth(csm.getReportTime()).intValue());
                if (endBalSheet != null) {
                    saveMessage(
                            request,
                            getText("finance.balSheet.existed", new String[]{getCurrentYear().toString(), getCurrentMonth().toString()},
                                    request.getLocale()));
                    endBalSheet = null;
                }
            }
    ...
    public String onSubmit(@ModelAttribute("corpBalanceSheetModel") CorpBalanceSheetModel corpBalanceSheetModel, BindingResult errors,
    ...
    private void saveCorpBalanceSheet(CorpBalanceSheetModel corpBalanceSheetModel, final HttpServletRequest request) throws Exception {
        ...
        if (isNewEndBalSheet) {
            ...
            CorporateBalanceSheet cbs = super.financeSheetManager.findCorporateBalanceSheet(null, cp, getYear(reportTime), getMonth(reportTime).intValue());
            if (cbs != null) {
                saveError(
                        request,
                        getText("finance.balSheet.existed", new String[]{getYear(reportTime).toString(), getMonth(reportTime).toString()},
                                request.getLocale()));
                return;
            }
        } else {
        ...
        if (isNewEndBalSheet) {
            saveMessage(
                    request,
                    getText("corpBalanceSheet.added", new String[]{endBalSheet.getReportYear().toString(), endBalSheet.getReportMonth().toString()},
                            locale));
        } else {
            saveMessage(
                    request,
                    getText("corpBalanceSheet.updated",
                            new String[]{endBalSheet.getReportYear().toString(), endBalSheet.getReportMonth().toString()}, locale));
        }
    }
